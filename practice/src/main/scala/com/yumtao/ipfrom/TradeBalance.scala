...
object TradeBalance {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val wbsNo2Amt = sc.textFile(FileUtils.getPathInProject("draw/wbs_draw_success.txt"))
      .map(_.split("\t"))
      .map(tmp => TradeObj(tmp(6), BigDecimal(tmp(1)).*(BigDecimal(100)).toInt))

    // ftp提现成功rdd2(流水号, 金额)
    val ftpNo2Amt: RDD[TradeObj] = getDrawRddFromFtp(sc)
  ...
  private def getDrawRddFromFtp(sc: SparkContext) = {
    sc.textFile(FileUtils.getPathInProject("draw/WTTX20190128.txt"))
      ...
      .filter(_._3.equals("0000")).map(tmp => TradeObj(tmp._1, tmp._2.toInt))
  }
}


case class TradeObj(val thirdNo: String, val amt: Int) extends Serializable